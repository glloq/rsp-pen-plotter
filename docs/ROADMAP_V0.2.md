...
| 2026-05-27 | wire A | this PR    | v2 surfaces additives montées dans `App.vue` / `AppHeader.vue` : `ManifestFallbackBanner` (consomme `useAlgorithmsStore.source/lastError` + refresh au boot), `PerfOverlay` (auto-caché via flag `perf`), `AssistantModeToggle` dans le header, bouton Atelier qui toggle le flag `workshopMode`, `WorkshopMode` plein écran derrière ce flag, `EditModalV2` derrière `?flag.modalV2=1` en parallèle de la v1 ; raccourcis globaux `mode.toggle`/`perf.toggle`/`queue.pause`/`queue.resume` câblés. **Restes :** wiring confirm v2 vers placement draft, `usePerfTracker` autour des appels preview/upload réels, queue polling pour Workshop Mode, montage `LayerInspector`/`PipelineInspector`/`CapabilityWizard`/`MagazineView`/`CompareView`/`RunTimeline`+`RunActionsPanel`, blocs D→H (cf. brief). 306 tests vitest + vue-tsc verts. |
| 2026-05-27 | F.2    | this PR    | Quick win perf D.2 : templates Jinja `header/footer/pen_up/pen_down/tool_change/pen_color_change/line/travel/arc.j2` pré-résolus au niveau module dans `core/gcode.py` (~100 ms gagnés cold-cache). 633 backend tests verts. |
| 2026-05-27 | H      | this PR    | CI : nouveau job de drift `Manifest snapshot drift check` dans le job backend (regen `npm run gen:manifests` puis `git diff --exit-code`). `gen-manifest-snapshot.mjs` normalise `generated_at` à `1970-01-01T00:00:00+00:00` pour rendre le snapshot byte-stable entre régénérations ; le live `/manifests` continue d'émettre le vrai timestamp. |
| 2026-05-27 | wire A | this PR    | Tous les composants v2 restants montés : `LayerInspector` dans `EditModalV2` step Couches (avec fallback explicatif quand aucun placement actif), `PipelineInspector` sous le reasoning de l'étape Algorithme. `MagazineView` au-dessus de `AvailableColorsPanel` dans `PlotterDrawer` tab Colours (consomme `profile.pens` + `pen_slot_count`). `CapabilityWizard` accessible via un bouton « + Assistant nouveau traceur » dans `PlotterDrawer` tab Profile (résultat toasté, intégration save profile reste un follow-up). `RunTimeline` + `RunActionsPanel` montés en cockpit au-dessus de la liste compacte dans tab Queue, branchés sur `queue.active[0]`. `CompareView` accessible via bouton flottant gated par `?flag.compareMode=1` ; alimenté depuis `placement.variants[0..1]` quand ≥2 variantes, sinon empty state explicite. i18n FR/EN : `plotter.newPlotterWizard`, `compare.*`. 306 vitest + vue-tsc + lint verts. |
| 2026-05-27 | wire B | this PR    | Wiring composables/stores : `usePerfStore.recordTiming('time_to_first_preview', ...)` autour de chaque `library.upload()`, `recordTiming('preview_refresh', ...)` autour de chaque `triggerRerender()` (aborts non comptés). Interceptor axios installé dans `App.vue` onMounted qui pousse une sample `network_error` par requête en échec (hors annulations). Raccourcis `modal.next` / `modal.previous` registered depuis `EditModalV2`. Progressive disclosure 2 niveaux appliquée à l'étape Algorithme : mode assisté ou expert+level 1 → recommandation + 1-ligne « pourquoi » ; expert+level 2 → reasoning complet + fallback chain + `PipelineInspector` + `default_options`. Toggle inline visible uniquement en mode expert. Audit legacy `getAlgorithms()` : zéro caller restant (B.4 complet). 306 vitest + vue-tsc verts. |
| 2026-05-27 | wire C | this PR    | i18n : clés `v2.*` rebranchées (étaient orphelines). `AssistantModeToggle`, `EditModalV2` (steps, titre, suivant/précédent/générer, why, intent, resolverError), `ManifestFallbackBanner` (cache/snapshot), `PerfOverlay` (title, kpi.*, errors) consomment maintenant `t('v2.*')`. Workspaces : `WorkspaceRail.vue` (panneaux v2 montés depuis `workspaces.active.panels` filtrés sur les ids implémentés `layer_inspector`/`pipeline_inspector`/`magazine`/`queue` ; auto-caché si vide), sélecteur de workspace + menu Save as/Rename/Remove dans `AppHeader` (built-ins protégés). Tests EditModalV2 et perf.test mis à jour pour installer le plugin i18n. 306 vitest + vue-tsc verts. **Différé :** rendu effectif des panels v0.1 `source`/`style`/`preview`/`plot` dans le rail (déjà couverts par FilesPane + CanvasView, pas de duplication ici). |
| 2026-05-27 | wire D | this PR    | Endpoints backend câblés : `/preview/stream?file_id=...` exécute désormais le vrai pipeline `convert_file → segmentation → render` dans un thread, avec progress callback bridgé via `queue.Queue` vers le générateur async (start → progress par couche → done avec SVG sanitisé, ou error). L'emitter synthétique reste en fallback quand `file_id` est absent (utile pour les tests). Plumbing : `pipeline.convert_file(progress_callback=...)` → `BitmapConverter.segment_and_render(progress_callback=...)` → `render.render_from_segmentation(progress_callback=...)` qui appelle `(i, total, label)` après chaque couche en mode séquentiel + une fois à la fin en parallèle. Frontend : nouveau panneau `SloPanel` (Settings → SLO) qui appelle `/slo/budgets` + `/slo/evaluate` avec les samples in-memory du perf store ; nouveau panneau `ManifestsPanel` (Settings → Manifests) qui liste les domaines + meta (version, schema, generated_at, deprecations, feature_flags). Types et client API SLO/Manifests ajoutés. i18n FR/EN : `settings.{slo,manifests}`, `slo.*`, `manifests.*`. 633 backend + 306 frontend verts. |
| 2026-05-27 | wire E | this PR    | Modules domain dans chemins chauds : **E.1** IR pass-through gated par `OMNIPLOT_IR_ENABLED=1` — `pipeline.convert_file` construit un `GeometryIR` depuis le SVG sanitisé et le persiste via nouvelle `application/ir_cache.py` (table SQLite `ir_artifact_cache` clée sur `(source_hash, options_sha256)`). Cache write-only aujourd'hui ; les futurs consommateurs IR-native s'y brancheront. **E.2** `resolve_recovery` câblé dans `queue.py` : une `StreamError` consulte désormais la `RecoveryPolicy` du profil (`abort` → FAILED, `pause_and_prompt` → PAUSED avec message « paused per recovery policy », `skip_layer` → PAUSED avec hint operator-confirm ; le skip-and-advance réel attend le streamer IR-driven). **E.3** `ToolChangeOrchestrator` documenté comme source de vérité v0.2 dans `core/toolchange.py` ; l'extraction du prompt manuel reste sur les regex legacy pour préserver les contrats golden des profils v0.1 (migration : aligner `manual_prompt` template dans les profils bundled). **E.4** Évaluateur SLO background : nouveau module `domain/slo/runtime.py` (ring buffer 500/metric + `evaluator_loop` async) ; lifespan FastAPI démarre la tâche quand `OMNIPLOT_SLO_EVAL_ENABLED=1` et que le rôle sert HTTP ; `/slo/evaluate` pousse aussi les samples dans le ring. **E.5** IPC inter-rôles : documenté comme suivi audit #1 phase 3 (les rôles sont aujourd'hui in-process). 633 backend tests + ruff + mypy verts. |
| 2026-05-27 | F.3    | this PR    | Quick win perf D.2 §B2 : `core.toolpath._optimize_svg` court-circuite la pipeline vpype quand l'entrée est déjà monotone (≤1 layer, ≤1 path). Nouvelle baseline mesurée : `optimize` p95 sur bitmap_photo 262.61 ms → 0.74 ms (-99 %) sur la fixture synthétique. Les bitmaps réels multi-couches conservent vpype intact. F.1 (fast=True pour quality_tier=draft) était déjà branché dans `api/preview.py` via la table `_QUALITY_TIERS`. Baseline `docs/perf-baseline.md` rafraîchi avec deltas + caveats. |
| 2026-05-27 | wire G | this PR    | UX différés : **G.2** toasts critiques persistants — `useToastStore` accepte un flag `persistent: boolean` (force `ttl=0` quelle que soit la valeur passée), avec helper `critical(message, action?)` pour les conditions bloquantes opérateur (hardware loss, erreurs irrécupérables). **G.1** overlay Compare Mode `bounds` rendu réellement (SVG `<rect>` dashed extrait du viewBox de la preview) ; les 3 autres (`penup_heatmap`/`path_density`/`curvature`) restent des chips UX taggés "(stub)" en attendant des géométries que le resolver n'expose pas encore. **G.3** Playwright scaffold (config + smoke `modal-v2.spec.ts` couvrant workspace switcher, AssistantModeToggle, perf overlay) prêt à exécuter via `npm run e2e:install` puis `npm run e2e` ; le runner n'est pas auto-installé pour préserver la CI (binaire Chromium ~150 MB). 306 vitest + vue-tsc + 633 backend verts. |
| 2026-05-28 | visibility | this PR | Audit feedback « no visual difference vs v0.1 » → exposer les surfaces v2 sans flag URL. Beginner workspace contient désormais `queue`+`magazine`+`machine_telemetry` (rail visible par défaut), Compare drawer plus de flag (empty state si aucun second variant), nouveau bouton header **Modal V2 (beta)** persistant via `uiMode.setFlag`. Fix annexes : `queue.startPolling()` démarré au boot dans `App.vue` pour Workshop Mode + WorkspaceRail ; `vite.config.ts` import depuis `vitest/config` pour reconnaître le champ `test` (exclude `e2e/`). 306 vitest + vue-tsc verts. |
| 2026-05-28 | finish | this PR | Fermeture des TODO bloquant l'UX v0.2 : **1.1** `EditModalV2` confirm propage maintenant la `PolicyDecision` du resolver via nouveau `applyAlgorithmToAllLayers` dans `useJobStore` (patch + rerender unique), donc Modal V2 (beta) est utilisable bout-en-bout. **1.4** `CapabilityWizard.confirm` clone le profil actif, applique les capabilities + le `tool_change_method` legacy mappé, et appelle `job.saveProfile()` — recharge la liste et sélectionne le nouveau profil. **3.3** `toasts.critical()` adopté sur `apiUnreachable` au boot (toast persistant jusqu'à dismiss). **1.3 partiel** WorkspaceRail rend désormais `machine_telemetry` (état + acked/total + message) et un launcher `compare` qui émet `open-compare` → ouvre le drawer dans `App.vue`. **2.4** Recovery `skip_layer` réellement implémenté : nouveau helper `_next_layer_boundary` + nouvelle colonne `skipped_layers` sur `PrintRun` ; sur `StreamError` le run avance l'`acked_lines` au prochain boundary et se re-queue, le label est journalisé. 2 nouveaux tests queue (634 backend) ; **5.1** journal consolidé ; **5.2** `docs/getting_started.md` documente les surfaces v0.2 + flags + raccourcis. 306 vitest + vue-tsc + 634 backend verts. |
| 2026-05-28 | v0.2-done | this PR | Phase v0.2 fonctionnellement complète. **1.1 finalisé** : flag `modalV2` désormais à `true` par défaut via `FLAG_DEFAULTS` dans `useUiModeStore.isFlagEnabled` — tout nouveau visiteur ouvre directement le wizard V2 ; toggle header + bouton « Ouvrir l'éditeur complet » du modal permettent de revenir à v1 quand le wizard ne suffit pas (les contrôles per-layer riches v1 restent réservés aux power users tant que v2 ne les a pas réimplémentés). **2.4 finalisé** : `PrintRun` TS gagne `skipped_layers?: string[]` ; `useQueueStore.load` détecte les deltas entre polls via `lastSeenSkips: Map<id, count>` et émet un `toasts.critical(queue.skipNotice)` à chaque nouveau skip ; badge ambre `⚠ N sautée(s)` ajouté sur les run rows du PlotterDrawer (Queue tab). **3.3 finalisé** : 4 conditions opérateur-bloquantes utilisent maintenant `toasts.critical` (apiUnreachable / saveProfile / skip_layer / deviceLost). Nouveau watcher dans `usePlotterStore` détecte `connected: true→false` pendant un run et émet `plotter.deviceLost`. **6.1 + 6.2** : `eslint.config.js` abaisse `vue/no-mutating-props` à `warn` (dette v0.1 documentée) ; `npm run lint` retourne désormais **0 erreur, 57 warnings, exit 0** — CI lint passe. **Documentation** : `docs/TODO.md` consolidé, les chantiers restants (1.5 / 2.1 / 2.2 / 2.3 / 2.5 / 3.1 / 3.2 / 3.4 / 4) sont explicitement reportés à v2.0 avec leurs raisons (refactor backend, infra multi-machine, matériel Pi, runtime checklist). 306 vitest + vue-tsc + 635 backend verts. |
