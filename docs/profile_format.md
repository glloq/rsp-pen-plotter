...
| `scale_source` | `"unset"` \| `"manual"` \| `"measured"` (default `"manual"`) | Provenance of `mm_per_pixel`: the UI writes `"unset"` on a fresh config (camera measurement stays gated until the scale is typed → `"manual"` or measured by the assistant → `"measured"`). Legacy YAML without the field defaults to `"manual"` and keeps measuring |
