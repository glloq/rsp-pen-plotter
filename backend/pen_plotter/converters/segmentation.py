...
def otsu(
    ...
    Always returns exactly two clusters: ``0`` = ink (the minority of
    pixels — the actual line content), ``1`` = paper (the majority).
    The threshold is chosen automatically so the between-class variance
    is maximised — no operator tuning, robust against scan exposure,
    JPEG noise and slight paper tint.

    Cluster 0 is always the minority side of the threshold so the
    function gives the same answer for **black-on-white** scans (ink is
    the dark minority) and **white-on-black** scans (ink is the bright
    minority). The synthesised palette ``[(0,0,0), (255,255,255)]``
    pairs with this convention so the downstream ``drop_background``
    filter always drops paper regardless of source polarity. In
    monochrome ink mode the rendered stroke colour comes from
    ``mono_ink_color`` anyway, so the synthesised palette only affects
    layer ordering / background drop, not what ends up on paper.

    This is the right segmentation for **monochrome line art**
    (mechanical drawings, schematics, hand-traced sketches) — k-means
    with 4 colours scatters the anti-aliased edge pixels of every thin
    stroke across 3 grey clusters, so each cluster's mask is sparse and
    the centerline / edges algorithms run on a broken skeleton,
    dropping detail.
    """
    grey = _greyscale(image)
    if grey.size == 0 or grey.min() == grey.max():
        # Degenerate single-tone image — no threshold to find. Still
        # return the documented 2-row palette so downstream code that
        # iterates `for cluster in range(palette.shape[0])` doesn't
        # behave differently on edge cases. Every pixel goes to the
        # "paper" cluster (1); the ink cluster (0) is empty but exists
        # in the palette so layer ordering / drop_background can still
        # apply uniformly.
        labels = np.ones(grey.shape, dtype=np.intp)
    ...
    dark_side = grey < threshold
    # Pick whichever side has fewer pixels as "ink" (label 0). Works for
    # both polarities: black-on-white (dark side is minority) and
    # white-on-black (bright side is minority, so we flip).
    if dark_side.sum() <= (~dark_side).sum():
        labels = np.where(dark_side, 0, 1).astype(np.intp)
    else:
        labels = np.where(dark_side, 1, 0).astype(np.intp)
    # Synthesised palette: (0,0,0) for ink, (255,255,255) for paper.
    # Two reasons not to use the actual per-cluster mean RGB:
    # 1. drop_background filters on palette luminance >= 0.92, so we
    #    need the paper cluster to be the bright one regardless of
    #    source polarity (a white-on-black scan with real means would
    #    put luminance=0 on the paper side, drop_background would keep
    #    it, and the pen would trace the background).
    # 2. ``mono_ink_color`` overrides the rendered stroke at draw time
    #    anyway, so the synthesised black/white only affects the
    #    drop-background decision — exactly what we need.
    palette = np.array([[0, 0, 0], [255, 255, 255]], dtype=np.uint8)
    return labels, palette
