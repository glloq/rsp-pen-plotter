...
async def test_lifecycle_create_list_patch_delete() -> None:
    ...
    async with _client() as client:
        created = await client.post("/available-colors", json={"hex": "#e91e63", "name": "Crimson"})
        ...
        renamed = await client.patch(f"/available-colors/{color_id}", json={"name": "Cherry Red"})
        assert renamed.status_code == 200
        assert renamed.json()["name"] == "Cherry Red"
        # Hex is unchanged when the patch only touches name.
        assert renamed.json()["hex"] == "#e91e63"

        moved = await client.patch(f"/available-colors/{color_id}", json={"position": 7})
...
async def test_hex_canonicalisation_and_dedup() -> None:
    ...
    """
    async with _client() as client:
        first = await client.post("/available-colors", json={"hex": "#ABC", "name": "Sky"})
        ...
        for shape in ("aabbcc", "#AABBCC", "#abc"):
            again = await client.post("/available-colors", json={"hex": shape, "name": ""})
...
async def test_dedup_updates_name_when_supplied() -> None:
    ...
    """
    async with _client() as client:
        original = await client.post("/available-colors", json={"hex": "#222222", "name": "Blakk"})
        original_id = original.json()["color_id"]

        corrected = await client.post("/available-colors", json={"hex": "#222222", "name": "Black"})
...
async def test_dedup_preserves_name_when_empty() -> None:
    ...
    """
    async with _client() as client:
        original = await client.post("/available-colors", json={"hex": "#000000", "name": "Black"})
        original_id = original.json()["color_id"]

        echo = await client.post("/available-colors", json={"hex": "#000000", "name": ""})
...
async def test_invalid_hex_returns_422() -> None:
    ...
    async with _client() as client:
        ...
        for bad in ("not a colour", "#12", "#12345", "#xyz", "#1234567"):
            resp = await client.post("/available-colors", json={"hex": bad, "name": ""})
...
async def test_stroke_width_default_create_and_patch() -> None:
    ...
    async with _client() as client:
        ...
        patched = await client.patch(f"/available-colors/{color_id}", json={"stroke_width_mm": 1.2})
        ...
        bad = await client.post("/available-colors", json={"hex": "#333333", "stroke_width_mm": 0})
...
async def test_patch_hex_to_existing_value_409s() -> None:
    ...
    """
    async with _client() as client:
        a = await client.post("/available-colors", json={"hex": "#111111", "name": "A"})
        b = await client.post("/available-colors", json={"hex": "#222222", "name": "B"})
...
async def test_patch_unknown_id_returns_404() -> None:
    async with _client() as client:
        resp = await client.patch("/available-colors/does-not-exist", json={"name": "nope"})
