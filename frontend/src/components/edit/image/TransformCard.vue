...
const isNeutral = computed(
  () =>
    props.preprocess.rotate_deg === 0 &&
    !props.preprocess.flip_h &&
    !props.preprocess.flip_v &&
    props.preprocess.crop === null,
)

function reset(): void {
  props.preprocess.rotate_deg = 0
  props.preprocess.flip_h = false
  props.preprocess.flip_v = false
  props.preprocess.crop = null
}
</script>
...
<template>
  <CollapsibleCard
    card-key="preprocess.transform"
    :title="t('preprocess.transform.title')"
    resettable
    :can-reset="!isNeutral"
