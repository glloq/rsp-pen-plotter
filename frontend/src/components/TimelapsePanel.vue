...
const { status, files, busy } = storeToRefs(timelapse)

const open = ref(false)

// Cameras available to record from (enabled + with a URL).
interface Cam {
  url: string
  label: string
}
...
watch(activeCameras, (list) => {
  if (selectedCam.value >= list.length) selectedCam.value = 0
})
...
async function start(): Promise<void> {
  const cam = activeCameras.value[selectedCam.value]
  if (!cam) return
  await timelapse.start(cam.url, intervalSeconds.value, fps.value, label.value.trim())
}
      ...
      <div v-else class="space-y-2">
        <div class="grid grid-cols-2 gap-2">
          <label class="col-span-2 block space-y-1">
            ...
            >
              <option v-for="(c, i) in activeCameras" :key="i" :value="i">{{ c.label }}</option>
            </select>
          </label>
