...
export const useJobStore = defineStore('job', () => {
  ...
  function syncPlacementToLibrary(p: Placement | null): void {
    ...
    // footprint (mm) — but never from a hidden "Edit from library" draft,
    // whose size is a placeholder, nor a zero/unsized placement — so
    // re-dropping the file restores the size the operator left it at.
    const footprint_mm =
      !p.is_library_draft && p.width_mm > 0 && p.height_mm > 0
  ...
  // picker). The signature keys on size only — a position-only move leaves
  // it unchanged, so dragging a placement around doesn't churn the
  ...
  watch(
    () => {
      const p = selectedPlacement.value
      if (!p || p.is_library_draft || !p.library_file_id) return null
  ...
  async function createPlacementFromLibrary(
    ...
    // A drop is never a draft, so a saved footprint reflects a real on-plan
    // size the operator chose — restore it (computeInitialLayout clamps it
    // to the current bed) instead of recomputing an auto-fit size.
    const preferredSize = options.asDraft ? null : saved?.footprint_mm
    const layoutPatch = computeInitialLayout(
      sourceBbox,
      placement.id,
      intrinsicPageSize(detail.upload_metadata),
