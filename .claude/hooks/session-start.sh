...
if command -v apt-get >/dev/null 2>&1; then
  ...
  apt-get install -y --no-install-recommends potrace ghostscript \
    || echo "warning: could not install potrace/ghostscript; related converters will be unavailable" >&2
fi
